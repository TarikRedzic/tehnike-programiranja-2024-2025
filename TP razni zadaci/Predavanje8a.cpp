    ...
    try
        {
            ...
            {
                p_ucenici.at(i) = std::make_shared<Ucenik>();
            }
            //std::vector<std::shared_ptr<Ucenik>> p_ucenici(broj_ucenika){std::make_shared<Ucenik>};
            try
                {
...
void IspisiJednogUcenika (const std::shared_ptr<Ucenik> p_ucenik)
{
    ...
    if (p_ucenik->prolaz)
        std::cout << " ima prosjek " << std::setprecision (3)
                  << p_ucenik->prosjek;
    else
