...
{
    auto temp = new Krug*[k.kapacitet]();
    try
    {
        for(int i{}; i < k.broj_krugova; i++) temp[i] = new Krug{*k.krugovi[i]};
        Dealociraj();
        krugovi = temp;
        broj_krugova = k.broj_krugova; kapacitet = k.kapacitet;
        temp = nullptr;
    }
    catch(const std::exception& e)
    {
        for(int i{}; i < k.broj_krugova; i++) delete temp[i];
        delete[] temp;
    }
    return *this;
}
...
Krugovi Krugovi::operator=(Krugovi &&k)
{
    if(&k != this)
    {
        for(int i{}; i < broj_krugova; i++) delete krugovi[i];
        delete[] krugovi;
...
void Krugovi::DodajKrug(double x, double y, double r)
{
    ...
    krugovi[broj_krugova++] = new Krug{x,y,r};
}
...
void Krugovi::IspisiKrugove() const
{
    ...
    std::sort(zasort, zasort + broj_krugova, [](Krug* k1, Krug* k2){
        return k1->r > k2->r;
    });
    for(int i{}; i < broj_krugova; i++)
