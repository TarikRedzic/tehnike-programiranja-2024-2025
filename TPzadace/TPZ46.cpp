...
class Biblioteka
{
    std::map<int, std::shared_ptr<Student>> clanovi;
    std::map<int, std::shared_ptr<Knjiga>> evidencija;
...
{
    for(auto& par: clanovi) {par.second = nullptr;}
    for(auto& par: evidencija) {par.second = nullptr;}
    evidencija.clear();
    clanovi.clear(); 
}
void Biblioteka::IspisiStudenta(const Student& s)
{
    std::cout<<"Ime i prezime: "<<s.ime<<" "<<s.prezime<<
    "\nGodina studija: "<<s.godina_studija<<
    "\nAdresa: "<<s.adresa<<
    "\nBroj telefona: "<<s.telefon<<std::endl;
}
...
{
    std::map<int, std::shared_ptr<Knjiga>> nova_evidencija;
    std::map<int, std::shared_ptr<Student>> novi_clanovi;
    try
    {
        for(auto& par: b.evidencija)
        {
            std::pair<int, std::shared_ptr<Knjiga>> n;
            n.first = par.first; n.second = std::make_shared<Knjiga>(*par.second);
            nova_evidencija.insert(n); n.second = nullptr;
        }
    }
    catch(const std::exception& e)
    {
        for(auto& par: nova_evidencija) par.second = nullptr;
        throw;
    }
    try
    {
        ...
        {
            std::pair<int, std::shared_ptr<Student>> n;
            n.first = par.first; n.second = std::make_shared<Student>(*par.second);
    ...
    {
        for(auto& par: novi_clanovi) par.second = nullptr;
        for(auto& par: nova_evidencija) par.second = nullptr;
        throw;
    }
...
void Biblioteka::RegistrirajNovogStudenta(int indeks, std::string ime, std::string prezime,
        int godina_studija, std::string adresa, std::string broj_telefona)
{
    std::pair<int, std::shared_ptr<Student>> st{indeks, 
        std::make_shared<Student>(Student{indeks,  ime, prezime, godina_studija, adresa, broj_telefona})};
    auto pretraga = clanovi.find(indeks);
    if(pretraga == clanovi.end()) clanovi.insert(st);
...
void Biblioteka::RegistrirajNovuKnjigu(int ev_broj, std::string naslov, std::string ime_pisca,
        std::string zanr, int godina_izdavanja)
{
    std::pair<int, std::shared_ptr<Knjiga>> k{ev_broj,
        std::make_shared<Knjiga>(naslov, ime_pisca, zanr, godina_izdavanja)};
    auto pretraga = evidencija.find(ev_broj);
    if(pretraga == evidencija.end()) evidencija.insert(k);
...
void Biblioteka::IzlistajStudente() const
{
    ...
    {
        std::cout<<"\nBroj indeksa: "<<par.first<<std::endl;
        IspisiStudenta(*par.second);
    }
}
...
void Biblioteka::PrikaziZaduzenja(int indeks)
{
    try
    {
        ...
        {
            if(par.second->DajPokKodKogaJe() != nullptr)
                if(par.second->DajPokKodKogaJe()->broj_indeksa == s.broj_indeksa)
                {
                    std::cout<<"\nEvidencijski broj: "<<par.first<<
                    "\nNaslov: "<<par.second->DajNaslov()<<
                    "\nPisac: "<<par.second->DajAutora()<<
