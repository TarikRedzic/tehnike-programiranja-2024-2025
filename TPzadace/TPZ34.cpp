...
std::vector<std::set<std::string>> Razvrstavanje(const std::vector<std::string> v, int k)
{
    ...
    for(auto s: v){imena.push_back(s);};
    auto tren = imena.begin();
    auto del = tren;
    for(int i{}, br_tim = 0; i < n; i++)
    {
        int vel = duzina(*tren);
        if(br_tim < (n % k) ? timovi.at(br_tim).size() == (n / k) + 1
            : timovi.at(br_tim).size() == n / k) br_tim++;
        timovi.at(br_tim).insert(*tren);
        tren = imena.erase(tren);
        if(tren == imena.end()) tren = imena.begin();
        for(int j{1}; j < vel; j++) if(++tren == imena.end()) tren = imena.begin();
    }
    return timovi;
}
