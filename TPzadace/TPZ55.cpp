...
template <typename TipEl> class Matrica 
{
    ...
    Matrica(Matrica &&m);
    Matrica(std::string ime, bool tip);
    ...
    template <typename Tip2>
    friend inline Matrica<Tip2> operator *(Tip2 d, const Matrica<Tip2> &m);
...
template <typename TipEl> 
    Matrica<TipEl>::Matrica(std::string ime, bool tip): 
        elementi(nullptr), br_redova(0)
{
    if(tip) ObnoviIzBinarneDatoteke(ime);
    else ObnoviIzTekstualneDatoteke(ime);
}

template <typename TipEl> void Matrica<TipEl>::SacuvajUTekstualnuDatoteku(std::string ime)
{
    std::ofstream fajl(ime);
    for(int i = 0; i < br_redova; i++)
        for(int j = 0; j < br_kolona; j++)
            fajl<<elementi[i][j]<<(j == br_kolona - 1 ? '\n' : ',');
}

template <typename TipEl> void Matrica<TipEl>::SacuvajUBinarnuDatoteku(std::string ime)
{
    std::ofstream fajl(ime, std::ios::binary);
    for(int i = 0; i < br_redova; i++)
        for(int j = 0; j < br_kolona; j++){
            fajl.write(reinterpret_cast<char*>(&elementi[i][j]), sizeof(TipEl));
            char znak = (j == br_kolona - 1 ? '\n' : ',');
            fajl.write(reinterpret_cast<char*>(&znak), sizeof(znak));
        }
            
}

template <typename TipEl> void Matrica<TipEl>::ObnoviIzTekstualneDatoteke(std::string ime)
{
    std::ifstream fajl(ime);
    if(!fajl) throw std::logic_error("Trazena datoteka ne postoji");
    TipEl unos;
    std::logic_error greska("Datoteka sadrzi besmislene podatke");
    //prva tura citanja
    int br_r{0}, max_k{0}, tren{};
    char znak;
    while(fajl>>unos)
    {
        tren++;
        if((znak = fajl.get()) == EOF)
        {
            if(br_r++ == 0) max_k = tren;
            else if(max_k != tren)
            {
                fajl.close();
                throw greska;
        ...
        else if(znak != ',')throw greska;
    }
    fajl.clear();
    //druga tura citanja
    fajl.close(); fajl.open(ime);
    TipEl** temp = new TipEl*[br_r]{};
    try
    {
        for(int i = 0; i < br_r; i++) temp[i] = new TipEl{};
    }
    catch(const std::exception& e)
    {
        for(int i = 0; i < br_r; i++) delete temp[i];
        delete[] temp;
    }
    ...
    br_kolona = max_k; br_redova = br_r;
    if(elementi != nullptr )DealocirajMemoriju(elementi, br_r);
    elementi = temp; temp = nullptr;
}

template <typename TipEl> 
    void Matrica<TipEl>::ObnoviIzBinarneDatoteke(std::string ime)
{
    std::ifstream fajl(ime, std::ios::binary);
    if(!fajl) throw std::logic_error("Trazena datoteka ne postoji");
    TipEl unos;
    std::logic_error greska("Datoteka sadrzi besmislene podatke");
    //prva tura citanja
    int br_r{0}, max_k{0}, tren{};
    char znak;
    while (fajl.read(reinterpret_cast<char*>(&unos), sizeof(TipEl)))
    {
        tren++;
        fajl.read(reinterpret_cast<char*>(&znak), sizeof(char));
        if(znak == EOF)
        {
            if(br_r++ == 0) max_k = tren;
            else if(max_k != tren)
    ...
    for(int i = 0; i < br_r; i++)
    {
        for(int j = 0; j < max_k; j++)
        {
            fajl.read(reinterpret_cast<char*>(&unos), sizeof(TipEl));
            temp[i][j] = unos;
            fajl.read(reinterpret_cast<char*>(&znak), sizeof(char));
        }
    }
    br_redova = br_r; br_kolona = max_k;
    if(elementi != nullptr)DealocirajMemoriju(elementi, br_r);
    elementi = temp; temp = nullptr;
}
